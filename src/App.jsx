...
const PUBLIC_FILE_ID = "1eOdZ0_TsQAS_LrAfVcEatNgST_5VNsm5"; // <--- ЗАМІНІТЬ НА ВАШ СКУПІЙОВАНИЙ FILE ID

// Секретний пароль для доступу до адмінки на сайті
const ADMIN_PASSWORD = "012596"; 
...
export default function App() {
  ...
  useEffect(() => {
    const fetchPublicDatabase = async () => {
      // Перевіряємо, чи вставив користувач реальний ID файлу замість плейсхолдера
      if (PUBLIC_FILE_ID && PUBLIC_FILE_ID !== '1aBcDeFgHiJkLmNoPqRsTuVwXyZ' && PUBLIC_FILE_ID !== '') {
        try {
          // Використовуємо всесвітній надійний CORS-проксі allorigins для завантаження контенту з Диска в браузер клієнта
          const driveUrl = `https://docs.google.com/uc?export=download&id=${PUBLIC_FILE_ID}`;
          ...
          if (response.ok) {
            ...
            console.log("Базу даних магазину успішно завантажено з хмари Google Drive!");
          }
        } catch (e) {
          console.error("Не вдалося завантажити хмарну базу даних. Працюємо на локальних копіях.", e);
        }
      }
    };
    fetchPublicDatabase();
  ...
  const handleAdminLogin = (e) => {
    ...
    if (passwordInput === ADMIN_PASSWORD) {
      ...
      // Ініціюємо завантаження скрипта Google
      if (!window.google) {
        const script = document.createElement('script');
        script.src = 'https://accounts.google.com/gsi/client';
        script.async = true;
  ...
  const syncWithGoogleDrive = async (passedToken = null, passedConfig = null) => {
    ...
    try {
      ...
      // Пошук існуючого файлу meat_store_db.json
      if (!fileId) {
      ...
      if (fileId) {
        // Оновлення файлу на Google Диску
        ...
        // Створення нового файлу з метаданими
        const metadata = {
            ...
            <div className="flex items-center gap-4">
              ...
              {gdriveConfig.isConnected && (
                <button
                  ...
                  disabled={isSyncing}
                  className="bg-zinc-850 hover:bg-zinc-800 border border-zinc-700 text-zinc-300 px-3 py-1 rounded-md font-bold flex items-center gap-1.5 transition-all disabled:opacity-50"
                >
                      ...
                      {order.status === 'new' && (
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            className="bg-zinc-850 hover:bg-zinc-800 text-zinc-400 hover:text-red-400 px-5 py-3 rounded-xl text-xs font-bold transition-all uppercase tracking-wider border border-zinc-800"
                          >
                            Скасувати
                          </button>
                        </div>
            ...
            {adminSubTab === 'gdrive' && (
              <div className="max-w-2xl mx-auto bg-zinc-900 rounded-3xl p-8 border border-zinc-850 shadow-xl">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold font-serif text-white">Google Drive Синхронізація</h3>
                      ...
                      {gdriveConfig.isConnected ? (
                        <>
                          ...
                          <button
                            ...
                            className="bg-zinc-800 hover:bg-zinc-700 text-red-450 font-bold py-3.5 px-6 rounded-xl text-xs transition-all border border-zinc-750"
                          >
                            Відключити
                          </button>
                        </>
