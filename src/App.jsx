...
export default function App() {
  ...
  const [siteSettings, setSiteSettings] = useState(() => {
    ...
    return saved ? JSON.parse(saved) : {
      ...
      // Налаштування ваших контактів для клієнтів
      contactPhone: '+380671234567',
      contactTelegram: 'vash_username',
      contactWhatsapp: '380671234567' // без плюса, лише цифри починаючи з коду країни
    };
  });
  ...
  const generateOrderMessage = (order) => {
    ...
    return `*Нове замовлення ${order.id}* від ${order.date}\n\n` +
           ...
           `💬 *Коментар:* ${order.customer.comment || 'немає'}\n\n` +
           `🥩 *Перелік м'яса:*\n${itemsText}\n\n` +
